<div @vigilancePoll('5s') class="space-y-6">
    ...
    <x-vigilance::ui.card class="overflow-hidden">
        <x-vigilance::ui.table class="v-table--sticky-end">
            <thead>
                {{-- Triage order. Eight columns plus the pinned actions cannot
                     fit a laptop, so the ones that answer "does this need me
                     now?" — state, how often, how recently — come before the
                     ones you read once you have decided to look. Message and
                     the 7d shape sit past the fold, where scrolling for them is
                     a deliberate act rather than the cost of a first glance. --}}
                <tr>
            ...
            <tbody>
                @forelse ($groups as $group)
                    @php
                        ...
                        // An all-zero week draws a flat line pinned to the
                        // baseline, which reads as a stray underscore rather
                        // than as "nothing happened" — say nothing instead.
                        $series = ($series && max($series) > 0) ? $series : [];
                    ...
                    <tr wire:key="group-{{ $group->id }}">
                        ...
                        <td class="max-w-[15rem] truncate v-muted" title="{{ $group->message }}">{{ \Illuminate\Support\Str::limit($group->message, 80) }}</td>
                        <td>
